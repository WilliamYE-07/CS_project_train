...
class _SeatingState extends State<SeatingPage> {
  ...
  _SeatingState(this.roomCode)  {
    ...
    db.collection("users").doc(AuthenticationHelper().uid).get().then((value) {
      ...
      List<dynamic> x = value.data()?["information"]; //gets the information for your current account

      //goes through the members list of the current room that we are in
      db.collection("rooms").doc(roomCode).collection("members").get().then((value) async {

        Map<String, List<dynamic>> xMembers = {}; //Sets up a list of Uid as keys and information as value
        ...
        for (var i in value.docs) {
          ...
          await db.collection("users").doc(i.id).get().then((v) {
            ...
            if (AuthenticationHelper().uid != i.id) {
              xMembers[i.id] = (v.data()?["information"]); //uid as a key and information as a value

            }

          });
        }
        String xMemb = jsonEncode(xMembers); //"Bank" this is all of the information for all the other members of this current room
        String X = jsonEncode(x); //"Target" this is all the information for the current user of the app
        ...
        http.get(Uri.parse('https://williamserver.bigphan.repl.co/getSittingRecommandation/$X/$xMemb')).then((r) async {
          print(r.statusCode);
